...
async fn should_transferer_un_seul_fichier() {
    // je garde le TempDir vivant jusqu'à la fin du test : le fichier doit
    // encore exister quand le transfert lit la source
    let dir = temp_dir("single");
    let data = write_random_file(&dir.path().join("probe.bin"), 2 * 1024 * 1024);
    let ui = run_transfer(vec![("probe.bin".into(), data)]).await;

    assert!(
        ui.is_completed("transfer-loopback"),
    ...
    assert_eq!(
        state.received[0].2,
        2 * 1024 * 1024,
        "octets reçus = taille du fichier"
    );
}
...
async fn should_transferer_plusieurs_fichiers_dans_un_lot() {
    ...
    for i in 0..3 {
        ...
        let data = write_random_file(&dir.path().join(format!("f{i}.bin")), 1024 * 1024);
...
async fn should_transferer_un_dossier() {
    ...
    let src_folder = root_path.join("src");
