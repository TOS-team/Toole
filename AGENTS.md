...
1. **Hooks `beforeDevCommand`/`beforeBuildCommand`** : Tauri les lance via la shell (`sh -c` sur Unix, `cmd /S /C` sur Windows) avec un `cwd` qui dépend de l'invocation (`desktop-app/src-tauri` via `tauri-action`/`cargo tauri build` → cwd `desktop-app`, `desktop-app/` → cwd `desktop-app/ui`, `desktop-app/ui/` → cwd `desktop-app/ui`). Ne les repasse jamais en `sh -c '…'`. La seule forme fiable partout et quel que soit le `cwd` est `npm --prefix ui run <script> || npm run <script>` : `--prefix ui` localise le package depuis n'importe quel répertoire (sinon `cd ui` réussit et `||` saute le build — c'est ce qui a fait échouer la release v2.1.0 en CI), et le fallback couvre le cas où l'on est déjà dans `ui/`.
...
- **Linux** : `WEBKIT_DISABLE_DMABUF_RENDERER=1` dans `main.rs` (workaround NVIDIA). Ne pas retirer.
