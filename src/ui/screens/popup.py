...
class ConnectionRequestPopup(ctk.CTkToplevel):
    def __init__(
        self, master, username: str, ip: str, on_accept: Callable, on_decline: Callable
    ):
        ...
        ctk.CTkLabel(
            self,
            text=f"'{username}' veut se connecter avec vous.",
        ...
        ctk.CTkButton(
            ...
            fg_color="#FF5252",
            hover_color="#FF1744",
            command=lambda: [on_decline(), self.destroy()],
        ).pack(side="left", padx=10)
...
class FileTransferRequestPopup(ctk.CTkToplevel):
    def __init__(
        ...
        ctk.CTkLabel(
            self, text=f"'{sender}' souhaite vous envoyer :", font=ctk.CTkFont(size=16)
        ).pack()
        ctk.CTkLabel(
            self, text=filename, font=ctk.CTkFont(size=18, weight="bold")
        ).pack()
