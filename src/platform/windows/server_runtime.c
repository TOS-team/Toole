...
//permet de multiplexer les sockets de presence et de hear pour une utilisation efficace
...
{
    if (!self || !master_lost || timeout_ms < 0)
    {
        return -1;
    }
...
int runtime_try_reconnect_from_devices(const device *liste, int nb, const info *self, info *master_out,int *socket_out)
{
    ...
    *socket_out = -1;
    ...
    for (int i = 0; i < nb; i++)
    {
        ...
        int fd_neighbor =
            connect_to(node->ip, (uint16_t)node->tcp_port);
...
{
    if (!self || !new_master || !socket_out || !next_state || nb < 0)
    {
        return -1;
    }
    ...
    *next_state = ELECTION;
    ...
    if (runtime_elect_master_from_devices(liste, nb, self, new_master) < 0)
    {
        return -1;
    }
